...
MODULE_FIXTURES_ERRATA = MappingProxyType(
    {
        ...
        'packages': ['duck-0.7-1.noarch.rpm', 'kangaroo-0.3-1.noarch.rpm'],
    }
)
...
"""The RPM packages listed as dependecy for the walrus 0.71 module."""
...
RPM_UNSIGNED_MODIFIED_FEED_URL = urljoin(
    PULP_FIXTURES_BASE_URL, 'rpm-unsigned-modified/')
"""The URL to an unsigned RPM repository.This repository does not have the
package `whale-0.2-1.noarch.rpm` present in :data:`RPM_UNSIGNED_FEED_URL`.
...
RPM_WITH_MODULES_MODIFIED_FEED_URL = urljoin(
    PULP_FIXTURES_BASE_URL, 'rpm-with-modules-modified/'
)
"""The URL to a modular RPM repository - missing dependencies.

This repository does not have the following packages:
